...
from effect import HealEffect, SlowEffect, AccelerateEffect
...
class Enemy(EngineMovingSprite):
    ...
    # Inside the Enemy class where the enemy dies (e.g., in the kill method):
    def drop_effect(self):
        print("DROP EFFECT")
        effect_chance = random.random()
        if effect_chance < 0.1:  # Adjust the probability as needed
            effects = [HealEffect(), SlowEffect(), AccelerateEffect()]
            effect = random.choice(effects)
            effect.position = self.rect.center
            effect.active = True
            self.engine.add_to_group(effect, "effects")
...
class Boar(Enemy):
    def __init__(self):
        super().__init__()
        self.speed = 3
        self.image_front = pygame.image.load("images/boar_front.png").convert_alpha()
        self.image_back = pygame.image.load("images/boar_back.png").convert_alpha()
        self.image_left = pygame.image.load("images/boar_left.png").convert_alpha()
        self.image_right = pygame.image.load("images/boar_right.png").convert_alpha()
        self.surf = self.image_front
        self.position = self.generate_position()
        self.rect = self.surf.get_rect(center=self.position[:2])
    ...
    # Add any additional methods specific to the Boar class
...
class Bird(Enemy):
    def __init__(self):
        super().__init__()
        self.speed = 4
        self.image_front = pygame.image.load("images/bird.png").convert_alpha()
        self.surf = self.image_front
        self.position = self.generate_position()
        self.rect = self.surf.get_rect(center=self.position[:2])
    ...
    # Add any additional methods specific to the Bird class
